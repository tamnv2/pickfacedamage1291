...
internal static class SessionUiBinder
{
    ...
    private static void ApplyButtonPermissions(Control root, FirebaseUserProfile profile)
    {
        ...
        {
            if (string.Equals(button.Text, "Đồng bộ lại", StringComparison.Ordinal) && !profile.HasPermission("sync_google")) button.Enabled = false;
            if ((button.Text.Contains("Kết nối Google", StringComparison.OrdinalIgnoreCase) || button.Text.Contains("Kiểm tra Drive", StringComparison.OrdinalIgnoreCase)) && !profile.HasPermission("sync_google")) button.Enabled = false;
            if (button.Text.Contains("ADMIN", StringComparison.OrdinalIgnoreCase)) button.Visible = false;
        }
    }
            ...
            while (!form.IsDisposed)
            {
                try
                {
                    ...
                    while (!form.IsDisposed && !reader.EndOfStream)
                    {
                        ...
                        {
                            if ((eventName == "put" || eventName == "patch") && !string.IsNullOrWhiteSpace(data)) await ApplySnapshotAsync();
                            eventName = null;
                            data = null;
                        }
                    }
...
internal sealed class MyAccountControl : UserControl
{
    ...
    public MyAccountControl()
    {
        ...
        var box = new GroupBox { Text = "Thông tin tài khoản", Dock = DockStyle.Top, AutoSize = true, AutoSizeMode = AutoSizeMode.GrowAndShrink, Padding = new Padding(16) };
        var form = new TableLayoutPanel { Dock = DockStyle.Top, AutoSize = true, AutoSizeMode = AutoSizeMode.GrowAndShrink, ColumnCount = 2, RowCount = 0, Padding = new Padding(8) };
        ...
        AddRow(form, "Họ tên hiển thị", _name);
        AddRow(form, "Email đăng ký", _email);
        AddRow(form, "Quyền", _role);
        ...
        var save = new Button { Text = "Lưu họ tên", AutoSize = true, Height = 36 };
        var reset = new Button { Text = "Gửi email đặt lại mật khẩu", AutoSize = true, Height = 36 };
        ...
        AddRow(form, "Thao tác", actions);
        _status.AutoSize = true;
        AddRow(form, "Trạng thái", _status);
        AddRow(form, "Ghi chú", new Label
        {
            AutoSize = true,
            MaximumSize = new Size(780, 0),
            ForeColor = Color.DimGray,
            Text = "Email đăng nhập không đổi trực tiếp ở bản này để tránh lệch giữa Firebase Authentication và hồ sơ ứng dụng. ADMIN không được xem hoặc biết mật khẩu của USER."
        });
    ...
    private static void AddRow(TableLayoutPanel form, string label, Control control)
    {
        ...
        form.Controls.Add(new Label { Text = label, AutoSize = true, Padding = new Padding(0, 8, 10, 14), Margin = new Padding(0, 4, 0, 4) }, 0, row);
        control.Margin = new Padding(3, 4, 3, 14);
        if (control is TextBox text) text.BackColor = text.ReadOnly ? Color.White : SystemColors.Window;
    ...
    private async Task SaveNameAsync()
    {
        ...
        if (session is null || session.OfflineMode) { MessageBox.Show(this, "Cần online để cập nhật thông tin tài khoản.", "Offline"); return; }
        var value = _name.Text.Trim();
        if (string.IsNullOrWhiteSpace(value)) { MessageBox.Show(this, "Họ tên không được để trống.", "Thiếu thông tin"); return; }
        try
        {
        ...
        catch (Exception ex) { MessageBox.Show(this, ex.Message, "Không cập nhật được", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
    }
    ...
    private async Task ResetPasswordAsync()
    {
        ...
        catch (Exception ex) { MessageBox.Show(this, ex.Message, "Không gửi được email", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
    }
}
