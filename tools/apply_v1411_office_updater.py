...
REPO = "tamnv2/pickfacedamage1291"
PROJECT = "pickface-damage-1291"
DRIVE_ROOT = "16jDCy5_Z1X5cKJyNPR1rn_ZqbExQSAeC"


def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise SystemExit(f"Missing required marker for {label}: {marker}")


updater_path = Path("src/PickfaceDamage1291/VersionUpdateServiceV2.cs")
text = updater_path.read_text(encoding="utf-8")

if "GatewayHttp = CreateGatewayHttpClient()" not in text:
    require(text, "    private static readonly HttpClient Http = CreateHttpClient();", "updater client")
    text = text.replace(
...
start_marker = "    public static async Task<ReleaseInfo?> GetLatestAsync"
end_marker = "    public static bool IsNewer"
require(text, start_marker, "GetLatestAsync")
require(text, end_marker, "IsNewer")
start = text.index(start_marker)
end = text.index(end_marker, start)
latest_block = r'''    public static async Task<ReleaseInfo?> GetLatestAsync(CancellationToken cancellationToken = default)
    {
...
'''
text = text[:start] + latest_block + text[end:]

start_marker = "    public static async Task DownloadPackageAsync("
end_marker = "    public static async Task<string> PrepareAutoUpdateAsync("
require(text, start_marker, "DownloadPackageAsync")
require(text, end_marker, "PrepareAutoUpdateAsync")
start = text.index(start_marker)
end = text.index(end_marker, start)
download_block = r'''    public static async Task DownloadPackageAsync(
        ReleaseInfo release,
        string destination,
...
'''
text = text[:start] + download_block + text[end:]

client_marker = '''    private static HttpClient CreateHttpClient()
    {
        var client = NetworkHttpClientFactory.Create(TimeSpan.FromMinutes(5), "PickfaceDamage1291-Updater");
        client.DefaultRequestHeaders.Accept.ParseAdd("application/vnd.github+json");
        return client;
    }'''
if "private static HttpClient CreateGatewayHttpClient()" not in text:
    require(text, client_marker, "CreateHttpClient")
    text = text.replace(
        client_marker,
        client_marker
        + r'''
...
main_path = Path("src/PickfaceDamage1291/MainFormV3.cs")
main = main_path.read_text(encoding="utf-8")
main = main.replace("Đang kiểm tra GitHub Release...", "Đang kiểm tra bản cập nhật (GitHub, có Google dự phòng)...")
main = main.replace("Chưa có GitHub Release hợp lệ để cập nhật tự động.", "Chưa có bản phát hành hợp lệ để cập nhật tự động.")
main = main.replace(
    "Có thể dùng nút Mở GitHub Releases để tải thủ công.",
    "Ứng dụng đã thử cả GitHub và Google Drive dự phòng. Hãy kiểm tra mạng rồi thử lại.",
)
main_path.write_text(main, encoding="utf-8")

project_path = Path("src/PickfaceDamage1291/PickfaceDamage1291.csproj")
project = project_path.read_text(encoding="utf-8")
for old, new in [
    ("<Version>1.4.10</Version>", "<Version>1.4.11</Version>"),
    ("<InformationalVersion>1.4.10</InformationalVersion>", "<InformationalVersion>1.4.11</InformationalVersion>"),
    ("<AssemblyVersion>1.4.10.0</AssemblyVersion>", "<AssemblyVersion>1.4.11.0</AssemblyVersion>"),
    ("<FileVersion>1.4.10.0</FileVersion>", "<FileVersion>1.4.11.0</FileVersion>"),
]:
...
print(f"Patched {REPO} for {PROJECT}; Drive root locked to {DRIVE_ROOT}.")
